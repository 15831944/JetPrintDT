...
*-------------------------------------------------------------------*/
BOOL CJetDxfDimView::GetDimRect(PENTITYHEADER pEntityHeader, PENTDIMENSION pDim, REALRECT* pRect)
{
	DIMSTYLE	DimStyle;
...
|  Output: TRUE if everything is ok                                 |
*-------------------------------------------------------------------*/
BOOL CJetDxfDimView::dimDrawArrowHead(OBJHANDLE BlockObjhandle, PENTITYHEADER pHeader, double cx, double cy, double Angle, double ArrowSize, LPCTSTR ArrowHeadName, int BLKNum)
{
	ENTLINE		Line;
	ENTSOLID	Solid;
	...
	{
		if ((strcmp(CW2A(ArrowHeadName), CW2A(ArrowHeads[i].ArrowHeadName)) == 0)
			&& (BLKNum == ArrowHeads[i].BLKNum))
		{
			if (strcmp(CW2A(ArrowHeads[i].Command), "LINE") == 0)
			{
				...
				drwAddEntity_Direct(BlockObjhandle, pHeader, &Line);
			}
			else if (wcscmp(ArrowHeads[i].Command, _T("SOLID")) == 0)
			{
				...
				drwAddEntity_Direct(BlockObjhandle, pHeader, &Solid);
			}
		}
	}
...
|  Output: TRUE if everything is ok                                 |
*-------------------------------------------------------------------*/
BOOL CJetDxfDimView::DimLinear(OBJHANDLE BlockObjhandle, PENTITYHEADER pDimHeader, PENTDIMENSION pDimension, PDIMSTYLE pDimStyle)
{
	BOOL			result;
	double			TextWidth;
	double			a, a1, a2, a3;
	...
	LPCTSTR			text = CA2W(pDimension->DimText);				// dim-line's text (dafault is NULL)
	...
	drwAddEntity_Direct(BlockObjhandle, &EntityHeader, &Point);
	...
	case 0: // Centered
		...
		TextWidth = (strlen(CW2A(text)) * pDimStyle->dimtxt * 0.3) + pDimStyle->dimgap * 2;
		...
		drwAddEntity_Direct(BlockObjhandle, &EntityHeader, &Line);
		...
		strcpy_s(Text.strText, pDimension->DimText);
		drwAddEntity_Direct(BlockObjhandle, &EntityHeader, &Text);
		break;
	...
	{
		// 1st Arrow Head
		result &= dimDrawArrowHead(BlockObjhandle, &EntityHeader, A1.x, A1.y, pi + atan(a3), pDimStyle->dimasz, CA2W(pDimStyle->dimblk1), 1);
		// 2nd Arrow Head
		result &= dimDrawArrowHead(BlockObjhandle, &EntityHeader, B1.x, B1.y, atan(a3), pDimStyle->dimasz, CA2W(pDimStyle->dimblk2), 2);
	}
	else if (B1.x<A1.x)
	{
		// 1st Arrow Head
		result &= dimDrawArrowHead(BlockObjhandle, &EntityHeader, A1.x, A1.y, atan(a3), pDimStyle->dimasz, CA2W(pDimStyle->dimblk1), 1);
		// 2nd Arrow Head
		result &= dimDrawArrowHead( BlockObjhandle, &EntityHeader, B1.x, B1.y, pi + atan(a3), pDimStyle->dimasz, CA2W(pDimStyle->dimblk2), 2);
	}
	else // (B1.x==A1.x)
	{
		// 1st Arrow Head
		result &= dimDrawArrowHead(BlockObjhandle, &EntityHeader, A1.x, A1.y, (B1.y>A1.y ? 0 : 1)*pi + atan(a3), pDimStyle->dimasz, CA2W(pDimStyle->dimblk1), 1);
		// 2nd Arrow Head
		result &= dimDrawArrowHead(BlockObjhandle, &EntityHeader, B1.x, B1.y, (B1.y>A1.y ? 1 : 0)*pi + atan(a3), pDimStyle->dimasz, CA2W(pDimStyle->dimblk2), 2);
	}
...
*-------------------------------------------------------------------*/
BOOL CJetDxfDimView::IsDimInViewableArea(PVIEW pView, RECT *pDimRect)
{
	return TRUE;
}
...
|  Output: Nothing                                                  |
*-------------------------------------------------------------------*/
void CJetDxfDimView::PlotDimension(HDC hdc, PENTITYHEADER pEntityHeader, PENTDIMENSION pDimension, double x = 0, double y = 0, double xScale = 1, double yScale = 1, double Rotation = 0)
{
	LAYER		Layer;
	//	LTYPE		LType;
	//	STYLE		Style;
	...
	if (drwFindTableType_Direct(TAB_LAYER, FIND_BYHANDLE, &Layer)>0)
	{
