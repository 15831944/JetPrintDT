...
CJetDxfTextView::CJetDxfTextView()
{
	m_pDrawing = NULL;
}
...
*-------------------------------------------------------------------*/
BOOL CJetDxfTextView::GetTextRect(PENTITYHEADER pEntityHeader, PENTTEXT pText, REALRECT* pRect)
{
	return TRUE;
}
...
*-------------------------------------------------------------------*/
BOOL CJetDxfTextView::IsTextInViewableArea(PVIEW pView, RECT *pTextRect)
{
...
*-------------------------------------------------------------------*/
UINT CJetDxfTextView::DecodeText(LPCTSTR TextIn, LPTSTR TextOut)
{
	UINT i = 0;
	UINT l = 0;
	UINT j = 0;
...
|  Output: Nothing                                                  |
*-------------------------------------------------------------------*/
void CJetDxfTextView::PlotText(HDC hdc, PENTITYHEADER pEntityHeader, 
							   PENTTEXT pText, double x = 0, double y = 0, 
							   double xScale = 1, double yScale = 1, double Rotation = 0)
{
	LAYER		Layer;
	//	LTYPE		LType;
	STYLE		Style;
	...
	wcscpy_s(logfont.lfFaceName, _T("Times New Roman"));
	logfont.lfWeight = FW_DONTCARE;
	logfont.lfOutPrecision = OUT_TT_PRECIS;
	logfont.lfCharSet = ANSI_CHARSET;
	logfont.lfQuality = NONANTIALIASED_QUALITY;
	logfont.lfPitchAndFamily = DEFAULT_PITCH | FF_DONTCARE;
	logfont.lfHeight = -MulDiv(int(pText->TextData.Height*yScale * m_pDrawing->View.PPU * m_pDrawing->View.ZoomLevel), GetDeviceCaps(hdc, LOGPIXELSY), 72);
	logfont.lfEscapement = int(pText->TextData.RotationAngle * 10);
	logfont.lfOrientation = logfont.lfEscapement;

	// Find Text Style
	Style.Objhandle = pText->TextData.TextStyleObjhandle;
	if (drwFindTableType_Direct(TAB_STYLE, FIND_BYHANDLE, &Style)>0)
	{
		if (pText->TextData.Height <= 0)
			logfont.lfHeight = -MulDiv(int(Style.LastHeightUsed*yScale * m_pDrawing->View.PPU * m_pDrawing->View.ZoomLevel), GetDeviceCaps(hdc, LOGPIXELSY), 72);
	}
	...
	if (abs(logfont.lfHeight)>1)
	{
		...
		UINT DecodedTextLen;
		DecodedTextLen = DecodeText(CA2W(pText->strText), CA2W(strDecodedText));
		...
		GetTextExtentPoint32(hdc, CA2W(strDecodedText), DecodedTextLen, &size);
		switch (pText->TextData.Justification)
		{
		case TJ_CENTER:
			uFormat |= TA_BOTTOM | TA_CENTER;
			rect.left = int( UnitToHorizontalPixel(pText->TextData.SecondAlignmentPoint.x*xScale)
				+ (size.cy*0.2)*sin(pText->TextData.RotationAngle*PI / 180));
			rect.bottom = int( UnitToVerticalPixel(pText->TextData.SecondAlignmentPoint.y*yScale)
		...
		case TJ_BOTTOMLEFT:
			...
			rect.left = UnitToHorizontalPixel(pText->TextData.SecondAlignmentPoint.x*xScale);
			rect.bottom = UnitToVerticalPixel(pText->TextData.SecondAlignmentPoint.y*yScale);
			break;
		...
		default:
			uFormat |= TA_BOTTOM | TA_LEFT;
			rect.left = int(UnitToHorizontalPixel(pText->Point0.x) + (size.cy*0.2)*sin(pText->TextData.RotationAngle*PI / 180));
			rect.bottom = int(UnitToVerticalPixel(pText->Point0.y) + (size.cy*0.2)*cos(pText->TextData.RotationAngle*PI / 180));
		}
