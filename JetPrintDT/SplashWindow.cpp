...
const int CSplashWindow::m_productNameLeftMargin = 720;			// distance from left side to place name, company, copyright and version
...
const int CSplashWindow::m_bodyLeftMargin = 720;				// distance from left side to place company name, copyright, version and comment
...
const int CSplashWindow::m_statusVerticalHeight = 30;			// maximum height of status string
...
COLORREF CSplashWindow::m_statusMessageTextColor = RGB(28,120,135);// color used for status message while starting (-1 ==> use application name color)
...
{	
	...
	//char *name = (char *)((const char *)nameStr.GetBuffer());

	// Get the size of the version information.
	DWORD verSize = GetFileVersionInfoSize(
		nameStr,	// pointer to filename string
	...
	if (verSize != 0)
	{
		...
		char* lplpBuffer;
		BOOL doThis = FALSE;
		doThis = VerQueryValue(buf, TEXT("\\VarFileInfo\\Translation"),
			(LPVOID*)&lplpBuffer, &ver);

		CString strSubBlock, strTranslation, strTemp;
		strTemp.Format(_T("000%x"), *((unsigned short int *)lplpBuffer));
		strTranslation = strTemp.Right(4);
		strTemp.Format(_T("000%x"), *((unsigned short int *)&lplpBuffer[2]));
		strTranslation += strTemp.Right(4);

		strSubBlock.Format(_T("\\StringFileInfo\\%s\\ProductName"), strTranslation);
		doThis =  VerQueryValue(buf, strSubBlock.GetBuffer(256), (LPVOID*)&lplpBuffer, &ver);
		strSubBlock.ReleaseBuffer();
		if ( doThis ) {
			//strncpy_s(buf1, (char *)lplpBuffer, ver);
			//m_productNameString = (CString)buf1;
			m_productNameString.Format(_T("%s"), lplpBuffer);
		}

		strSubBlock.Format(_T("\\StringFileInfo\\%s\\CompanyName"), strTranslation);
		doThis = VerQueryValue (buf, strSubBlock.GetBuffer(256), (LPVOID*)&lplpBuffer, &ver);
		strSubBlock.ReleaseBuffer();
		if ( doThis ) {
			//strncpy_s(buf1, (char *)lplpBuffer, ver);
			//m_companyNameString = (CString)buf1;
			m_companyNameString.Format(_T("%s"), lplpBuffer);
		}

		strSubBlock.Format(_T("\\StringFileInfo\\%s\\LegalCopyright"), strTranslation);
		doThis = VerQueryValue(buf, strSubBlock.GetBuffer(256), (LPVOID*)&lplpBuffer, &ver);
		strSubBlock.ReleaseBuffer();
		if ( doThis ) {
			//strncpy_s(buf1, (char *)lplpBuffer, ver);
			//m_copyrightString = (CString)buf1;
			m_copyrightString.Format(_T("%s"), lplpBuffer);
		}

		strSubBlock.Format(_T("\\StringFileInfo\\%s\\Comments"), strTranslation);
		doThis = VerQueryValue(buf, strSubBlock.GetBuffer(256), (LPVOID*)&lplpBuffer, &ver);
		strSubBlock.ReleaseBuffer();
		if ( doThis ) {
			//strncpy_s(buf1, (char *)lplpBuffer, ver);
			//m_commentsString = (CString)buf1;
			m_commentsString.Format(_T("%s"), lplpBuffer);
		}
