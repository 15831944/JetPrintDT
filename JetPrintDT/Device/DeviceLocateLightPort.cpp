...
#include "DeviceLocateLightPort.h"


CDeviceLocateLightPort::CDeviceLocateLightPort()
{
	m_OpenFlag = false;
	m_nLightLuminence = 0;
	m_nPortChannel = 0;
...
void CDeviceLocateLightPort::ClosePort()
{
	if (this->m_OpenFlag)
	{
...
bool CDeviceLocateLightPort::OpenPort(CString comPortName)
{
...
DWORD CDeviceLocateLightPort::WriteSerial(BYTE * chBuf, DWORD dwLength)
{
...
DWORD CDeviceLocateLightPort::ReadSerial(BYTE* chBuf, DWORD dwLength)
{
  DWORD nLen;
  COMSTAT comStat;
  DWORD dwErrorFlags;
...
bool CDeviceLocateLightPort::SendData(BYTE byAddress, BYTE byCommand, BYTE byValue, BYTE byTrigger)
{
...
DWORD CDeviceLocateLightPort::GetBufferLength()
{
...
bool CDeviceLocateLightPort::InitLightPort()
{
  CString portName = _T("");
  portName.Format(_T("COM%d"),LOCATE_LIGHT_PORT);
  bool bResult = false;
...
int CDeviceLocateLightPort::GetLightLuminence()
{
	return m_nLightLuminence;
}

int CDeviceLocateLightPort::GetPortNumber(CString portName)
{
	int nResult = -1;
	if(portName.IsEmpty())
		return nResult;
