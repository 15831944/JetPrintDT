...
void CSkinButton::DrawButtonTitle()
{
	if(m_bRePain == FALSE)
	{
		CClientDC  dc(this);
...
/*
void CSkinButton::SetRGBColor(int R,int G,int B)//设置背景三个色调
{
//	m_ColorR = R;
//	m_ColorG = G;
//	m_ColorB = B;
...
void CSkinButton::DrawButtonNullPicture()
{
	if(m_bRePain == FALSE)
	{
		CPoint posText(0,0);
...
void CSkinButton::DrawButtonScroll()
{
	if(m_bRePain == FALSE)
	{
		CClientDC  dc(this);
...
void CSkinButton::SetParentWnd(HWND hWnd)
{
	m_ParentWnd = hWnd;
...
void CSkinButton::SetRantageDraw(BOOL bDraw)
{
	m_bDrawRantage = bDraw;
}


void CSkinButton::SetScrollDraw(int bDraw ,int nNum)
{
...
void CSkinButton::SetMovePain(BOOL bDraw)
{
    m_bRePain = bDraw;
}

BOOL CSkinButton::MoveWndBegin(fListMoveWnd lpfListMoveWnd,LPARAM lPARAM)
{
	if(m_state == DOWN)
	{
		CPoint	point;
...
void CSkinButton::SetButtonText(CString str)
{
	m_strTextOut = str;
...
void CSkinButton::SetImage(HBITMAP nNormalID, HBITMAP nHoverID, HBITMAP nDownID, HBITMAP nDisableID)
{
	if(nNormalID==NULL)
		return;
...
void CSkinButton::SetIcons(int nIconIn, int nIconOut,BOOL bDrawIcon)
{
	HICON		hIconIn			= NULL;
	HICON		hIconOut		= NULL;
...
void CSkinButton::SetIcons(CString  strMouseOut,CString strMouseIn,BOOL bDrawIcon)
{
