...
#include "CPUInfoHeader.h"
#pragma comment(lib,"SingleTon/WinRing0.lib")
// --------------------------------------------------------
//
	...
	if (Features.HasThermal){
		__try {
			...
			for (int i = 0; i < nProcessors; i++){
				//CPU Temp
				result = SetThreadAffinityMask(GetCurrentThread(), i);
				Rdmsr(0x19c, &eax, &edx);//read Temperature
				SetThreadAffinityMask(GetCurrentThread(), result);
				cpuTemp[i] = 100 - ((eax & 0x007f0000) >> 16);
				
			}
			
			// SSE FP capable processor.
			Features.HasThermal = true;
		}
		// A generic catch-all just to be sure...
		__except (1) {
