...
CChartAxis::CChartAxis() 
 : m_pParentCtrl(NULL), m_bIsHorizontal(true), m_bIsInverted(false), 
   ...
   m__maxValue(0), m_MinValue(0), m_UnzoomMax(0), m_UnzoomMin(0), 
...
void CChartAxis::SetMinMax(double Minimum, double Maximum)
{
	...
	m__maxValue = m_UnzoomMax = Maximum;
	RefreshScrollBar();
	m_pParentCtrl->RefreshCtrl();
}
...
bool CChartAxis::RefreshAutoAxis()
{
	...
	if ( (SeriesMax!=m__maxValue) || (SeriesMin!=m_MinValue) )
...
long CChartAxis::ValueToScreenStandard(double Value) const
{
	...
	Offset = (int)floor( (Value - m_MinValue) * GetAxisLenght()/(m__maxValue-m_MinValue) );
	if (m_bIsHorizontal)
	{
		if (!m_bIsInverted)
			retVal = (m_StartPos + Offset);
...
double CChartAxis::ScreenToValue(long ScreenVal) const
{
	...
	return ( (AxisOffset * 1.0 / GetAxisLenght()*(m__maxValue-m_MinValue)) + m_MinValue);
}
...
void CChartAxis::PanAxis(long PanStart, long PanEnd)
{
	...
	SetZoomMinMax(m_MinValue+Shift,m__maxValue+Shift);
}
...
void CChartAxis::SetZoomMinMax(double Minimum, double Maximum)
{
	...
	if ( (Maximum - Minimum) < m_dZoomLimit)
		m__maxValue = m_MinValue + m_dZoomLimit;
	else
		m__maxValue = Maximum;
	RefreshScrollBar();
}
								   ...
								   int& iCurrentStep)
{
	...
	if ((m__maxValue-m_MinValue) == 0 || (SeriesMax-SeriesMin)==0 )
	{
		iTotalSteps = 1;
		iCurrentStep = 1;
	}
	...
	{
		double dStep = (m__maxValue - m_MinValue) / 10.0;
									 ...
									 bool bScrollInverted)
{
	...
	if (bScrollInverted)
		SetZoomMinMax(m_MinValue-dOffset,m__maxValue-dOffset);
	else
		SetZoomMinMax(m_MinValue+dOffset,m__maxValue+dOffset);
