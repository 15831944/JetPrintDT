...
long CChartLogarithmicAxis::ValueToScreenStandard(double Value) const
{
	...
	Offset = (int)floor((log10(Value)-log10(m_MinValue)) * GetAxisLenght()/(log10(m__maxValue)-log10(m_MinValue)) );
	if (m_bIsHorizontal)
	{
		if (!m_bIsInverted)
...
double CChartLogarithmicAxis::ScreenToValue(long ScreenVal) const
{
	...
	return (pow(10.0,(AxisOffset *1.0 / GetAxisLenght()*(log10(m__maxValue)-log10(m_MinValue)) ) + log10(m_MinValue)) );
}
...
void CChartLogarithmicAxis::PanAxis(long PanStart, long PanEnd)
{
	...
	SetZoomMinMax(m_MinValue*Factor,m__maxValue*Factor);
}
...
void CChartLogarithmicAxis::GetScrollbarSteps(int& iTotalSteps, int& iCurrentStep)
{
	...
	{
		double dStep = pow(m__maxValue/m_MinValue,0.1);
												...
												bool bScrollInverted)
{
	...
	if (bScrollInverted)
		SetZoomMinMax(m_MinValue/dFactor,m__maxValue/dFactor);
	else
		SetZoomMinMax(m_MinValue*dFactor,m__maxValue*dFactor);
}
