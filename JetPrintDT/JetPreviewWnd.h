...
#define  JETPANELWND _T("JetPanelWnd")

class CJetPreviewWnd : public CWnd
{
	DECLARE_DYNAMIC(CJetPreviewWnd)

public:
	enum {RECT_WIDTH = 4};

	CJetPreviewWnd();
	virtual ~CJetPreviewWnd();
	...
	void updatelocatecameraPoint(CPoint pos);
	void drawlocatecamera(CPoint pos, CClientDC* pDc);

	void updatejetPoint(CPoint pos);
	void drawjetPoint(CPoint pos,CClientDC *pDc);

	void updateprintorignpos(CPoint pos);
	void drawprintorignpoint(CPoint pos, CClientDC* pDc);
...
private:
	...
	CPoint locateCameraPoint;
	CPoint jetPoint;
	CPoint printOrignPoint;
